...
from seynse_logger.settings import logger_settings
...
def set_global_logs(values):
    ...
    values: <dict>
        ...
        must be present in SEYNSE_LOGGER[`LOGGING_FORMAT`] settings

    Returns
    -------

    Exception: If `key` not present in SEYNSE_LOGGER[`LOGGING_FORMAT`] or
               `request` is empty
