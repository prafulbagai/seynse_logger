...
Settings for the Seynse logging are all namespaced in the SEYNSE_LOGGER setting.
...
This module provides the `SeynseLoggerSettings` object, that is used to access
Seynse Logger settings, checking for user settings first, then falling
...
USER_SETTINGS = getattr(settings, "SEYNSE_LOGGER", None)
...
class SeynseLoggerSettings(object):
    """
    A settings object, that allows Seynse Logger settings to be accessed as properties.
    ...
    def __getattr__(self, attr):
        ...
        try:
            ...
            if attr not in self.defaults.keys():
                raise ImproperlyConfigured("Invalid SeynseLogger setting: %r." % (attr))
    ...
    def validate_setting(self):
        ...
        for attr in self.mandatory:
            if not attr in self.user_settings or not self.user_settings.get(attr):
                raise ImproperlyConfigured("SeynseLogger setting: %r is mandatory" % attr)
            # ImproperlyConfigured

        # validate logger format
        if not isinstance(self.user_settings.get('LOGGING_FORMAT', {}), dict):
            raise ImproperlyConfigured("SeynseLogger settings LOGGING_FORMAT must be a %r" % type({}))


logger_settings = SeynseLoggerSettings(USER_SETTINGS, DEFAULTS, MANDATORY)
